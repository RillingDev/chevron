var Chevron = function () {
  ...
  const typeFactory = function (moduleContent, dependencies) {
    ...
    const dependenciesArr = Array.from(dependencies); //First value gets ignored by calling 'new' like this, so we need to fill it with something

    dependenciesArr.unshift(0); //Apply into new constructor by binding applying the bind method.
  ...
  const Chevron = class {
     ...
     */
    constructor() {
      const _this = this; //Instance container


      _this.$ = new Map(); // Adds default types
    ...
    extend(typeName, constructorFunction) {
      const _this = this; //stores type as set with name into instance
    ...
    set(moduleName, dependencies, content, constructorFunction) {
      ...
      const _module = [false, content]; //Add init-fn

      _module.push(() => {
        const constructedDependencies = dependencies.map(dependencyName => _this.get(dependencyName)); //Calls constructorFunction on the module
